...
class ECODatabaseLoader:
    ...
    def load_database(self, force_download: bool = False) -> List[Dict[str, Any]]:
        ...
        if os.path.exists(cached_file) and not force_download:
            try:
                logger.info(f"Loading ECO database from cache: {cached_file}")
        ...
        try:
            logger.info("Downloading ECO database from Hugging Face...")
            ...
            try:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info("Successfully downloaded via HTTPS")
                    else:
            ...
            logger.info(f"Saved ECO database to cache: {cached_file}")
            ...
            if os.path.exists(sample_file):
                logger.info(f"Using sample ECO database: {sample_file}")
    ...
    def _log_database_info(self, is_sample=False):
        ...
        logger.info(f"Loaded {db_type} ECO database with {len(self.eco_data)} entries")
        logger.info(f"Volume distribution: {volumes}")
        ...
        for code in specific_codes:
            ...
            if entries:
                logger.info(f"Found {len(entries)} entries for ECO code {code}")
                for e in entries[:2]:  # Log first two entries
                    moves_count = len(e.get('uci', '').split())
                    logger.info(f"  - {e.get('name', 'No name')}: {moves_count} moves")
    ...
    def get_theoretical_moves(self, eco_code: str) -> List[str]:
        ...
        logger.info(f"Looking up theoretical moves for ECO code: {standard_eco} (original: {eco_code})")
        ...
        if matches:
            logger.info(f"Found {len(matches)} entries for ECO code {standard_eco}")
            for i, match in enumerate(matches[:2]):  # Log first 2 matches
                moves = match.get('uci', '').split()
                logger.info(f"  - {match.get('name')}: {len(moves)} moves")
        ...
        logger.info(f"No match for {standard_eco}, trying volume: {volume}")
        ...
        if volume_matches:
            logger.info(f"Found {len(volume_matches)} entries for volume {volume}")
            # Return the moves from the first match
    ...
    def get_opening_name(self, eco_code: str) -> str:
        ...
        # Log info about the lookup
        logger.info(f"Looking up opening name for ECO code: {standard_eco} (original: {eco_code})")
